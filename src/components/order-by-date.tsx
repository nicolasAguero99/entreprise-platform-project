...
export default function OrderByDate ({ text }: { text: string }): JSX.Element {
  ...
  return (
    <button className='flex gap-2 items-center uppercase' onClick={() => { handleOrderBy(OrderTypes.DATE_ORDER) }}>{text}<OrderIcon isActive={active} /></button>
  )
}
