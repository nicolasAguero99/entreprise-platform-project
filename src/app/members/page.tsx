...
// Types
import { type SearchParams, type MembersAndPaginationProps, type OrderTypes } from '@/types/types.d'
...
export default async function MembersPage ({ searchParams }: { searchParams: SearchParams }): Promise<JSX.Element> {
  ...
  const { data, paginationPages, prev, next }: MembersAndPaginationProps = await getMembersAndPages(searchValue, Number(page), orderValue)

  return (
    <main className="flex flex-col flex-1 px-8 py-4">
      <h1 className="text-3xl font-semibold mt-20 lg:mt-4 mb-8">Members</h1>
